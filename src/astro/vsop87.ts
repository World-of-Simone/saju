/**
 * Truncated VSOP87 (Meeus, "Astronomical Algorithms" 2nd ed., Appendix III) for the Earth's
 * heliocentric ecliptic longitude L and radius vector R. These replace the low-precision
 * equation-of-center model and bring apparent solar longitude to well under an arcminute,
 * i.e. solar-term instants accurate to seconds rather than ~12 minutes.
 *
 * Each series term is A·cos(B + C·τ), τ = Julian millennia (TT) since J2000. A is in units of
 * 1e-8 radian (for L) or 1e-8 AU (for R). Series are summed and combined as a polynomial in τ.
 */

type Term = readonly [A: number, B: number, C: number];

// ---- Earth heliocentric longitude L (radians × 1e-8) ----
const L0: Term[] = [
...
function sumSeries(series: Term[], tau: number): number {
  let s = 0;
  for (const [a, b, c] of series) s += a * Math.cos(b + c * tau);
  return s;
}

function evalPoly(seriesList: Term[][], tau: number): number {
  let total = 0;
  let power = 1;
  for (const series of seriesList) {
    total += sumSeries(series, tau) * power;
    power *= tau;
  }
  return total;
}

/** Earth heliocentric longitude in radians (VSOP87, of-date dynamical frame). */
export function earthLongitudeRad(tauMillennia: number): number {
  return evalPoly([L0, L1, L2, L3, L4, L5], tauMillennia) / 1e8;
}

/** Earth–Sun distance in AU. */
export function earthRadiusAU(tauMillennia: number): number {
  return evalPoly([R0, R1, R2, R3, R4], tauMillennia) / 1e8;
}
